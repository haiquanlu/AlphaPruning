...
from prune_utils import prune_convnext, prune_vit, prune_vit_ww, prune_convnext_ww, check_sparsity
...
def get_args_parser():
    ...
    parser.add_argument("--prune_granularity", type=str, default='layer')
    parser.add_argument("--blocksize", type=int, default=1)
    
    parser.add_argument("--save_dir", type=str, default=None)
    parser.add_argument("--metric_cache", type=str, default='./metric_cache')
    parser.add_argument("--WW_metric", type=str, default='alpha_mid')
...
def main(args):
    ...
    if "convnext" in args.model:
        ...
        if "small" in args.model or "tiny" in args.model:
            pass
        else:
            checkpoint = torch.load(args.resume, map_location='cpu')
    ...
    calib_data_outlier = copy.deepcopy(calib_data)
    
    ################################################################################
    if args.sparsity != 0:
        with torch.no_grad():
            if "convnext" in args.model:
                if args.prune_metric == "magnitude" or args.prune_metric == "wanda":
                    prune_convnext(args, model, calib_data, device)
                elif args.prune_metric == "magnitude_ww" or args.prune_metric == "wanda_ww":
                    prune_convnext_ww(args, model,calib_data, device)
            elif "vit" in args.model:
                if args.prune_metric == "magnitude" or args.prune_metric == "wanda":
                    prune_vit(args, model, calib_data, device)
                elif args.prune_metric == "magnitude_ww" or args.prune_metric == "wanda_ww":
                    prune_vit_ww(args, model, calib_data, device)
            elif "deit" in args.model:
                if args.prune_metric == "magnitude" or args.prune_metric == "wanda":
                    print("begin pruning.")
                    prune_vit(args, model, calib_data, device)
    ...
    with open(f"{args.save_dir}/{args.sparsity}.txt", "w") as f:
        print(f"Accuracy of the network on {len(dataset_val)} test images: {test_stats['acc1']:.5f}%", file=f, flush=True)
